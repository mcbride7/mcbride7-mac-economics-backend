...
def insert_data_conflict(symbol, provider_a, value_a, provider_b, value_b,
                          diff_pct, tolerance_pct, status):
    """Logs a cross-source comparison result — 'agree' or 'conflict' — every
    time two providers are checked against each other for the same symbol.
    This is an append-only audit log (never upserted), so you can see the
    full history of how often sources agree, not just the latest check."""
    conn = get_conn()
    cur = conn.cursor()
    now = datetime.now(timezone.utc).isoformat()
    cur.execute(
        ...
        INSERT INTO data_conflicts
            (symbol, provider_a, value_a, provider_b, value_b, diff_pct, tolerance_pct, status, checked_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """),
        (symbol, provider_a, value_a, provider_b, value_b, diff_pct, tolerance_pct, status, now),
    )
    conn.commit()
    conn.close()
...
def get_recent_releases_all(limit_per_indicator=1, max_indicators=100):
    """Latest release row per distinct indicator — powers the Calendar
    page's 'Recent Releases' view without needing to know indicator names
    ahead of time."""
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        _q("SELECT country, indicator, reference_date, value, unit FROM economic_releases er "
           "WHERE reference_date = (SELECT MAX(reference_date) FROM economic_releases "
           "WHERE indicator = er.indicator) "
           "ORDER BY reference_date DESC LIMIT ?"),
        (max_indicators,),
    )
    result = _rows_to_dicts(cur)
...
def get_recent_conflicts(limit=30):
    ...
    cur = conn.cursor()
    cur.execute(
        _q("SELECT symbol, provider_a, value_a, provider_b, value_b, diff_pct, "
           "tolerance_pct, status, checked_at FROM data_conflicts "
